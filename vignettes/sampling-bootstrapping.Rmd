...
```{r cacao-ratings, eval = animate, fig.width=6, fig.asp = 1/2}
library(ggplot2)
library(gganimate)
...
```{r cacao-ratings-fade, eval = animate, fig.width=6, fig.asp = 1/2}
cacao %>% filter(location %in% c("Canada", "U.S.A.")) %>%
  ggplot(aes(rating, location)) +
  geom_point(
    position = position_jitter(height = 0.3, width = 0.05), 
...
```{r bootstrap-demo, eval = animate, fig.width = 6, fig.asp = 3/4}
set.seed(69527)
