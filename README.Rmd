...
The name comes from the German word "Ungewissheit" which means uncertainty.
...
The functions `bootstrap_summarize()` and `bootstrap_do()` are drop-in equivalents for dplyr's `summarize()` and `do()` but perform the respective action multiple times on bootstrapped data. This is convenient to generate hypothetical outcomes plots.
